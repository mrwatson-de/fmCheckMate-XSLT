...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	XSL-File: XMSS_Text_PrintVerboseLikePdf.xsl
	...
	<xsl:param name="delimiter1" select="concat($RETURN,'    ','[ ')"/>
	<xsl:param name="delimiter2" select="': '"/>
	<xsl:param name="delimiter3" select="concat(' ]',$RETURN,'    ','[ ')"/>
	...
	<!--
	 ! Steps
	 !-->
	<xsl:param name="pScriptStepHidePasswords" select="'False'"/>
	<!--
	 ! Rendering Options
	 !-->
	<xsl:param name="pVerbose" select="'True'"/>
