...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:date="http://exslt.org/dates-and-times" version="1.0">
	...
	XSL-File: XMSC_Text.Tab_ScriptsCalledMostFrequently.xsl
	...
	2014-07-15 MrW: Version 1.0 ... using the current() Xpath function!
	-->
	<!-- ===== HEAD ===== -->
	...
	<xsl:template match="/">
		...
		<xsl:for-each select="//Script[not(ancestor::Step)]">
			<xsl:sort select="count(//Step[@id='1' and @enable='True']/Script[@id=current()/@id and not(FileReference)])" order="descending" data-type="number"/>
			<!-- -->
