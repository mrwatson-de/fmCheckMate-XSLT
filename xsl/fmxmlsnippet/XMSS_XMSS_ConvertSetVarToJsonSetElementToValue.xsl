...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	XSL-File: XMSS_XMSS_ConvertSetVarToJsonSetElementToValue.xsl
	...
	<xsl:template match="/fmxmlsnippet">
		...
		<xsl:variable name="lines">
			<xsl:for-each select="Step[@id='141' and @enable='True']">
				...
				<xsl:variable name="calc" select="Value/Calculation/text()"/>
				<xsl:variable name="line" select="concat('[&quot;',$jsonKey,'&quot;',' ; ',$calc,' ; JSONString]')"/>
				<!-- -->
