...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exsl="http://exslt.org/common" version="1.0" exclude-result-prefixes="exsl">
	...
	XSL-File: XMTH_Text.Lines_ListThemeDetails.xsl

	XML-Source: fmxmlsnippet.xml
	XML-Grammar: fmxmlsnippet
	XML-Content: one or more FileMaker Themes
	
	Lists the details of the FileMaker Theme (for a human to read).
	...
	<xsl:template match="/fmxmlsnippet/Theme">
		...
		<xsl:value-of select="$newrecord"/>
		<xsl:value-of select="$newrecord"/>
		<xsl:value-of select="concat($indent,'Base-ID',$delimiter,@base_id)"/>
		<xsl:value-of select="$newrecord"/>
		<xsl:value-of select="concat($indent,'Base-Version',$delimiter,@base_version)"/>
		<xsl:value-of select="$newrecord"/>
		<xsl:value-of select="$newrecord"/>
	...
	<xsl:template match="namedstyles">
		...
		<xsl:for-each select="*">
			...
			<xsl:value-of select="concat($indent,text(),$delimiter,name(),$newrecord)"/>
		</xsl:for-each>
		<xsl:value-of select="$newrecord"/>
	</xsl:template>
	<!-- CSS -->
	<xsl:template match="CSS">
		...
		<xsl:value-of select="$newrecord"/>
		<!--
		<xsl:value-of select="'CSS - normalized Text:'"/>
		<xsl:value-of select="$newrecord"/>
		<xsl:value-of select="$newrecord"/>
		...
		<xsl:value-of select="$newrecord"/>
		 -->
		<!--
		<xsl:value-of select="'CSS - original Text:'"/>
		<xsl:value-of select="$newrecord"/>
		<xsl:value-of select="$newrecord"/>
