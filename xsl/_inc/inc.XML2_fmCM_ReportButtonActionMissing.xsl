...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	XSL-File: inc.XML2_fmCM_ReportButtonActionMissing.xsl

	XML-Source: fmxmlsnippet.xml
	XML-Grammar: fmxmlsnippet
	XML-Content: copied layout objects
	
	Include file in a layout analysis to report buttons lacking actions.
	...
	-->
	<!-- ===== TEMPLATES ===== -->
	<xsl:template name="XML2_fmCM_ReportButtonActionMissing">
		<xsl:param name="ErrorLevel" select="$K_FMCM_WARNING"/>
		<!-- -->
		<xsl:variable name="Event" select="'Button Action &lt;Missing&gt;'"/>
		<xsl:variable name="Result">
			<xsl:for-each select="//Object[@type='Button' and not(ButtonObj/Step)]">
				<!-- string-length(@name) -->
				<xsl:call-template name="fmCM.Results.writeResultForCurrentNode">
					...
					<xsl:with-param name="Result">
						...
						<xsl:value-of select="' has no action.'"/>
					</xsl:with-param>
				</xsl:call-template>
			</xsl:for-each>
		</xsl:variable>
