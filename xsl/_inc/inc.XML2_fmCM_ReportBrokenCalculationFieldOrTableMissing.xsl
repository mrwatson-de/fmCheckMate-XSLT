...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	XSL-File: inc.XML2_fmCM_ReportBrokenCalculationFieldOrTableMissing.xsl

	XML-Source: fmxmlsnippet.xml
	XML-Grammar: fmxmlsnippet
	XML-Content: copied layout objects
	
	Include file in a layout analysis to report broken calculations referencing a missing field or table.
	
	===== CHANGES HISTORY =====
	(c) russell@mrwatson.de 2020
	2018-06-04 MrW: Version 1.0
	-->
	<!-- ===== NEEDS ===== -->
	<!--
	...
	<xsl:template name="XML2_fmCM_ReportBrokenCalculationFieldOrTableMissing">
		<xsl:param name="ErrorLevel" select="$K_FMCM_ERROR"/>
		<!-- -->
		<xsl:variable name="Event" select="'Broken Calculation (&lt;Field Missing&gt; or &lt;Table Missing&gt;)'"/>
		<xsl:variable name="Result">
			<xsl:for-each select="//Calculation[contains(.,'&lt;Table Missing&gt;') or contains(.,'&lt;Field Missing&gt;')]">
				...
				<xsl:call-template name="fmCM.Results.writeResultForCurrentNode">
					...
					<xsl:with-param name="Result">
						...
						<xsl:variable name="thing">
							<xsl:choose>
								<xsl:when test="contains(.,'&lt;Field Missing&gt;')">
						...
						<xsl:value-of select="concat(' has a ',$QUOT,$ParentElementName,$QUOT,' calculation with a &lt;',$thing,' Missing&gt;: ')"/>
						<xsl:value-of select="."/>
					</xsl:with-param>
				</xsl:call-template>
			</xsl:for-each>
