...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	XSL-File: inc.XML2_fmCM_ReportBrokenInactiveValueLists.xsl
	...
	-->
	<!-- ===== TEMPLATES ===== -->
	<xsl:template name="XML2_fmCM_ReportBrokenInactiveValueLists">
		<xsl:param name="ErrorLevel" select="$K_FMCM_WARNING"/>
		<!-- -->
		<xsl:variable name="Event" select="'Inactive Broken Value List &lt;Missing&gt;'"/>
		<!-- Analysis -->
		<xsl:variable name="Result">
			<xsl:for-each select="//Object[contains('06',FieldObj/@displayType) and FieldObj/DDRInfo/ValueList/@name='']">
				<xsl:call-template name="fmCM.Results.writeResultForCurrentNode">
					...
					<xsl:with-param name="Result">
						...
						<xsl:value-of select="concat(' - inactive value list is &lt;missing&gt; {id:',FieldObj/DDRInfo/ValueList/@id,'}.')"/>
					</xsl:with-param>
				</xsl:call-template>
			</xsl:for-each>
		</xsl:variable>
