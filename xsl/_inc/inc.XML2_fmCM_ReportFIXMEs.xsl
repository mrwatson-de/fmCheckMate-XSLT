...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	XSL-File: inc.XML2_fmCM_ReportFIXMEs.xsl

	XML-Source: fmxmlsnippet.xml
	XML-Grammar: fmxmlsnippet
	XML-Content: copied layout objects
	
	Include file in a layout analysis to report occurrences of the word FIXME.
	...
	-->
	<!-- ===== TEMPLATES ===== -->
	<xsl:template name="XML2_fmCM_ReportFIXMEs">
		<xsl:param name="ErrorLevel" select="$K_FMCM_ERROR"/>
		<!-- -->
		<xsl:variable name="Event" select="'FIXME'"/>
		<xsl:variable name="Result">
			<xsl:for-each select="//Object[contains(TextObj/ParagraphStyleVector/Style/Data,'FIXME')]">
