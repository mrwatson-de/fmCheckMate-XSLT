...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	XSL-File: inc.XML2_fmCM_ReportReferencesToExternalFiles.xsl

	XML-Source: fmxmlsnippet.xml
	XML-Grammar: fmxmlsnippet
	XML-Content: copied layout objects
	
	Include file in a layout analysis to report external file references.
	...
	-->
	<!-- ===== TEMPLATES ===== -->
	<xsl:template name="XML2_fmCM_ReportReferencesToExternalFiles">
		<xsl:param name="ErrorLevel" select="$K_FMCM_WARNING"/>
		<!-- -->
		<xsl:variable name="Event" select="'External File Reference'"/>
			...
			<xsl:for-each select="//FileReference">
				<xsl:call-template name="fmCM.Results.writeResultForCurrentNode">
					...
					<xsl:with-param name="Result">
						...
						<xsl:value-of select="' references external file: '"/>
