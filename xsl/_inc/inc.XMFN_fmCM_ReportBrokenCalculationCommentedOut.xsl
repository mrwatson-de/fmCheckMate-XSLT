...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	XSL-File: inc.XMFN_fmCM_ReportBrokenCalculationCommentedOut.xsl

	XML-Source: fmxmlsnippet.xml
	XML-Grammar: fmxmlsnippet
	XML-Content: copied function objects
	
	Include file in a function analysis to report broken commented out calculations.
	...
	<xsl:template name="XMFN_fmCM_ReportBrokenCalculationCommentedOut">
			...
			<xsl:for-each select="//Calculation[substring(.,1,2)='/*' and substring(.,string-length(.) - 1)='*/']">
				...
				<xsl:call-template name="fmCM.Results.writeResultForCurrentNode">
					...
					<xsl:with-param name="Result">
						<xsl:apply-templates select="parent::CustomFunction" mode="toString"/>
