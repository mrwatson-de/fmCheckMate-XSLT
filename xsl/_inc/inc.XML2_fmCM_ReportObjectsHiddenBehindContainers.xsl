...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	XSL-File: inc.XML2_fmCM_ReportObjectsHiddenBehindContainers.xsl

	XML-Source: fmxmlsnippet.xml
	XML-Grammar: fmxmlsnippet
	XML-Content: copied layout objects
	
	Include file in a layout analysis to report objects hidden behind containers.
	...
	-->
	<!-- ===== TEMPLATES ===== -->
	<!-- XML2_fmCM_ReportObjectsHiddenBehindContainers -->
	<xsl:template name="XML2_fmCM_ReportObjectsHiddenBehindContainers">
		<xsl:param name="ErrorLevel" select="$K_FMCM_WARNING"/>
		<!-- -->
		<xsl:variable name="Event" select="'Hidden Object (Behind Container)'"/>
					...
					<xsl:if test="number(Bounds/@left) &gt;= $left and number(Bounds/@top) &gt;= $top and number(Bounds/@right) &lt;= $right and number(Bounds/@bottom) &lt;= $bottom">
						<xsl:call-template name="fmCM.Results.writeResultForCurrentNode">
							...
							<xsl:with-param name="Result">
								<xsl:apply-templates select="." mode="toString"/>
								<xsl:value-of select="' is COMPLETELY HIDDEN behind container '"/>
